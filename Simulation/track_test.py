...
def get_monitors(tune_real, tune_imag, n_turns, slicer, Q_x, Q_y):
    ...
    filename = '/home/vgubaidulin/Documents/PhD/Data/Stability_scans/test_scan_chroma/'
    ...
    slice_monitor = SliceMonitor(filename=slice_filename, n_steps=n_turns, slicer=slicer)
    return bunch_monitor, slice_monitor
if __name__=='__main__':
    ...
    chroma = Chromaticity(Qp_x=[5], Qp_y=[5])
    filename = '/home/vgubaidulin/Documents/PhD/Data/Stability_scans/test_scan_chroma/'
    # dQcoh_real = 1e-3*np.linspace(-0.5, 0.5, 21)
    # dQcoh_imag = 1e-3*np.linspace(0, 0.17, 18)
    ...
    print(dQcoh_real*1e3)
    print(dQcoh_imag*1e3)    
    ...
    for r in dQcoh_real:
        ...
        for i in dQcoh_imag:
            ...
            print('Damping rate: {0:.0f}, Phase: {1:.0f}'.format(dampingrate, phase))
            ...
            trans_map = TransverseMap(s, alpha_x, beta_x, D_x, alpha_y, beta_y, D_y, Q_x, Q_y, [chroma])
            trans_one_turn = [m for m in trans_map]
            map_ = trans_one_turn  + [long_map, damperx, dampery]
            bunch_monitor, slice_monitor = get_monitors(r, i, n_turns, slicer, Q_x, Q_y)
